        ...
        GeometryReader{ geometry in
            NavigationView{
                VStack{
                    ...
                    HStack{
                        Spacer()
                        Text("Cartão Master 1234").font(.custom("SF UI Text", size: 13))
                        Spacer()
                        Text("R$\(self.paymentamount)").font(.custom("SF UI Text", size: 13))
                       Spacer() }.padding(.top,-geometry.size.width/2.0)
                    Rectangle().path(in: CGRect(x: 20, y: 0, width: geometry.size.width - 40, height: 2)).foregroundColor(Color(#colorLiteral(red: 0.2549019754, green: 0.2745098174, blue: 0.3019607961, alpha: 1))).padding(.top, -geometry.size.width/2.3)
                    HStack{
                        Spacer()
                        Text("Total pago").font(.custom("SF UI Text", size: 17))
                        Spacer()
                        Text("R$\(self.paymentamount)").font(.custom("SF UI Text", size: 17))
                        Spacer()
                    }.padding(.top, -geometry.size.width/1.1)
